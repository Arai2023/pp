...
# creating Poison class that will decrease the life of our snake if it collides with it
class Poison:
    def __init__(self, x, y):
        self.location = Point(x, y)
...
class Snake:
    ...
    def check_super_food_collision(self, super_food):
        if self.points[0].x != super_food.x:
            return False
        if self.points[0].y != super_food.y:
            return False
        return True
...
while running:
    ...
    if snake.check_super_food_collision(superfood):
        # eat_sound.play()
        SCORE += 5
        snake.points.append(Point(superfood.x, superfood.y))
